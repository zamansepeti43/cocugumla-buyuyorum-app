  ...
  return (
    <div className="page profile-page">
      ...
      <div className="profile-layout">
        <section className="profile-card"><div className="section-heading"><div><h2>Çocuk profilleri</h2><p>{data.children.length} profil</p></div><Link to="/child/new" className="icon-text-button"><Plus size={18} /> Yeni çocuk</Link></div><div className="children-list">{data.children.map((child) => { const age = calculateAge(child.birthDate); const isActive = child.id === activeChild?.id; return <div className={`child-row ${isActive ? 'active' : ''}`} key={child.id}><button className="child-select" onClick={() => setActiveChild(child.id)}><span className="profile-avatar">{childNameInitial(child.name)}</span><span><strong>{formatChildName(child.name)}</strong><small>{age.label} · {age.ageGroup}</small></span>{isActive ? <i><Check size={16} /> Aktif</i> : <ChevronRight size={19} />}</button><button className="delete-button" onClick={() => remove(child.id, child.name)} aria-label={`${formatChildName(child.name)} profilini sil`}><Trash2 size={17} /></button></div> })}</div></section>

      ...
      <div className="profile-layout">
        <section className="profile-card">
          <div className="section-heading profile-heading">
            <div><h2>Çocuk profilleri</h2><p>{data.children.length} profil</p></div>
                ...
                return (
                  <div className={`child-row ${isActive ? 'active' : ''}`} key={child.id}>
                    <button className="child-select" onClick={() => setActiveChild(child.id)} aria-pressed={isActive}>
                      ...
                      <span className="child-copy"><strong>{formatChildName(child.name)}</strong><small>{age.label} · {age.ageGroup}</small></span>
                      {isActive ? <i><Check size={16} /> Aktif</i> : <ChevronRight size={19} />}
                    </button>
                    <div className="child-actions">
                      <Link to={`/child/${child.id}/edit`} className="profile-action edit" aria-label={`${formatChildName(child.name)} profilini düzenle`}><Pencil size={16} /></Link>
                      <button className="delete-button" onClick={() => remove(child.id, child.name)} aria-label={`${formatChildName(child.name)} profilini sil`}><Trash2 size={17} /></button>
                    </div>
                  </div>
                )
              })}
        ...
        <aside className="settings-card">
          ...
          <div className="setting-row"><ShieldCheck /><div><strong>Yerel ve gizli</strong><span>Profil ve ilerleme verileri yalnızca bu tarayıcıda saklanır.</span></div></div>
          <div className="setting-row"><UserRound /><div><strong>Aktif profil</strong><span>{activeChild ? formatChildName(activeChild.name) : 'Seçilmiş profil yok'}</span></div></div>
